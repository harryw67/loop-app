...
//
// Default platform fee is 15%. If the renter was referred by someone, the
// fee drops to 13% total, split as: renter pays 5% less, the referrer gets
// a 5% credit, and Loop nets the remaining 3%.
//
...
async function createPaymentHolds(supabase, rental) {
  ...
  let referrer = null;
  if (renterProfile.referred_by) {
    const { data: r } = await supabase.from('profiles').select('*').eq('referral_code', renterProfile.referred_by).maybeSingle();
    referrer = r;
  }

  const rentalChargeAmount = referrer ? Math.round(listing.price_cents * 0.95) : listing.price_cents;
  const applicationFee = referrer ? Math.round(listing.price_cents * 0.03) : Math.round(listing.price_cents * 0.15);
  ...
  const rentalIntent = await stripe.paymentIntents.create({
    ...
    off_session: true,
    confirm: true,
    capture_method: 'manual', // holds the charge — only actually moves money on capture
    transfer_data: ownerProfile?.stripe_account_id ? { destination: ownerProfile.stripe_account_id } : undefined,
    application_fee_amount: applicationFee, // Loop's cut, taken automatically on capture
  });
  ...
  if (referrer) {
    ...
    await supabase.from('profiles').update({ referral_credit_cents: (referrer.referral_credit_cents || 0) + credit }).eq('id', referrer.id);
  }
