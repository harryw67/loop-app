...
export async function PATCH(req, { params }) {
  ...
  const { name, category, size, color, gender, occasion, price, deposit, description, photos, distance, care_instructions, lat, lng, min_days, max_days, rentable, purchasable, buy_price } = body;
  ...
  if (rentable !== undefined) update.rentable = rentable;
  if (purchasable !== undefined) update.purchasable = purchasable;
  if (buy_price !== undefined) update.buy_price_cents = buy_price ? Math.round(parseFloat(buy_price) * 100) : null;
  if (gender !== undefined) update.gender = gender;
  if (occasion !== undefined) update.occasion = occasion;
  if (min_days !== undefined) update.min_days = parseInt(min_days);
  if (max_days !== undefined) update.max_days = Math.min(parseInt(max_days), 6); // see cap note in POST route
...
export async function DELETE(req, { params }) {
  ...
  if (listing.owner_id !== user.id) {
    ...
    if (!profile?.is_admin) return NextResponse.json({ error: 'Not your listing' }, { status: 403 });
  }

  // soft delete — keeps history intact for any existing rentals referencing it.
  // admin actions use the service-role client so this works regardless of
  // whose listing it is, bypassing the owner-only RLS policy.
  const admin = supabaseAdmin();
  const { error } = await admin.from('listings').update({ active: false }).eq('id', params.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
