...
export async function GET(req) {
  ...
  function buildBaseQuery(countOption) {
    let query = supabase
      .from('listings')
      .select('*, profiles(full_name), favorites(count)', countOption ? { count: 'exact' } : undefined)
      .eq('active', true)
      .eq('sold', false)
    ...
    if (genders) query = query.in('gender', genders.split(','));
...
export async function POST(req) {
  ...
  const { name, category, size, color, gender, occasion, price, deposit, description, photos, care_instructions, lat, lng, min_days, max_days, rentable, purchasable, buy_price } = body;

  if (!name || !category || !size || !description) {
    return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
  }
  const isRentable = rentable !== false; // defaults true
  const isPurchasable = !!purchasable;
  if (!isRentable && !isPurchasable) {
    return NextResponse.json({ error: 'Must be available to rent, buy, or both' }, { status: 400 });
  }
  if (isRentable && (!price || !deposit)) {
    return NextResponse.json({ error: 'Price and deposit required to rent' }, { status: 400 });
  }
  if (isPurchasable && !buy_price) {
    return NextResponse.json({ error: 'Buy price required' }, { status: 400 });
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      category,
      size,
      price_cents: isRentable ? Math.round(parseFloat(price) * 100) : 0,
      deposit_cents: isRentable ? Math.round(parseFloat(deposit) * 100) : 0,
      rentable: isRentable,
      purchasable: isPurchasable,
      buy_price_cents: isPurchasable ? Math.round(parseFloat(buy_price) * 100) : null,
      description,
      ...
      // hard-capped at 6 days regardless of what's requested — a deposit
      // hold on a card typically expires around 7 days, and this needs to
      // survive approval-to-handoff lag + the rental itself + return lag,
      // all inside that window
      max_days: Math.min(max_days ? parseInt(max_days) : 6, 6),
      color: color || null,
      gender: gender || null,
  ...
  // flag obviously out-of-scope content for a human to review — doesn't
  // block the listing, just gets it in front of you fast
  if (containsFlaggedContent(name) || containsFlaggedContent(description)) {
    const admin = supabaseAdmin();
    await admin.from('reports').insert({
      reporter_id: user.id, reported_user_id: user.id,
      reason: `AUTO-FLAG: listing "${name}" (id: ${data.id}) contains potentially prohibited content — needs review.`,
    });
  }
