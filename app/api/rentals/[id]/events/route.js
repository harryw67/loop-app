  ...
  if (kind === 'approve' || kind === 'decline') {
    ...
    if (held) {
      ...
      await supabase.from('rental_events').insert({
        ...
        payload: { text: "Payment can't be held yet — either the renter needs to add a card, or the owner needs to finish connecting a payout account in their profile.", system: true },
      });
    }
  ...
  if (kind === 'code_confirmed') {
    ...
    try {
      if (!rentalIntentId) {
        ...
        if (!held) return NextResponse.json({ error: "Can't complete this handoff yet — either add a payment method, or the owner needs to finish connecting a payout account in their profile." }, { status: 400 });
...
async function createPaymentHolds(supabase, rental) {
  ...
  if (!ownerProfile?.stripe_account_id) return null; // owner hasn't connected a payout account yet
