...
export async function POST(req, { params }) {
  ...
  if (kind === 'request_to_rent') {
    ...
    const isBuy = rental.transaction_type === 'buy';
    ...
    await supabase.from('rentals').update({
      stage: 'pending',
      start_date: isBuy ? null : body.start_date,
      end_date: isBuy ? null : body.end_date,
      expires_at,
    }).eq('id', rental.id);
    const { data } = await supabase.from('rental_events')
      .insert({ rental_id: rental.id, kind: 'message', actor_id: user.id, payload: {
        text: isBuy
          ? `Wants to buy this for $${(listing.buy_price_cents / 100).toFixed(0)} — waiting on the owner's approval.`
          : `Requested to rent this for ${formatDateShort(body.start_date)} to ${formatDateShort(body.end_date)} — waiting on the owner's approval.`,
      } })
      .select().single();
    return NextResponse.json({ event: data });
  }
  ...
  if (kind === 'approve' || kind === 'decline') {
    ...
    // auto-decline other pending requests on this listing — if this was a
    // purchase, the item is gone entirely so every other request gets
    // declined; if it was a rental, only ones overlapping these dates do
    ...
    for (const other of overlapping || []) {
      const shouldDecline = rental.transaction_type === 'buy'
        || (other.start_date && other.end_date && rental.start_date <= other.end_date && rental.end_date >= other.start_date);
      if (shouldDecline) {
        await supabase.from('rentals').update({ stage: 'declined' }).eq('id', other.id);
        await supabase.from('rental_events').insert({
          rental_id: other.id, kind: 'message', actor_id: null,
          payload: { text: rental.transaction_type === 'buy' ? 'This item was sold to someone else.' : 'This item got booked by someone else for those dates.', system: true },
        });
      }
    }
  ...
  if (kind === 'cancel') {
    ...
    // uses the admin client and verifies a row actually changed — a plain
    // session-bound update here could silently do nothing under RLS with
    // no error thrown, leaving the message below posted with no real
    // cancellation behind it
    const adminForCancel = supabaseAdmin();
    const { data: cancelledRow, error: cancelErr } = await adminForCancel
      .from('rentals')
      .update({ stage: 'cancelled', cancelled_by: role, cancelled_at: new Date().toISOString() })
      .eq('id', rental.id)
      .select()
      .maybeSingle();

    if (cancelErr || !cancelledRow) {
      return NextResponse.json({ error: 'Could not cancel this — try again.' }, { status: 500 });
    }
    ...
    if (wasApproved) {
      const { data: p } = await adminForCancel.from('profiles').select('cancellation_count').eq('id', user.id).single();
      await adminForCancel.from('profiles').update({ cancellation_count: (p?.cancellation_count || 0) + 1 }).eq('id', user.id);
    }
  ...
  if (kind === 'code_confirmed') {
    ...
    // once a sale actually hands off, the item is gone for good — mark it
    // sold immediately so no one else can buy or rent it, regardless of how
    // the 1-hour "flag an issue" window and eventual settlement play out
    if (rental.transaction_type === 'buy') {
      await supabase.from('listings').update({ sold: true }).eq('id', rental.listing_id);
      await supabase.from('rental_events').insert({
        rental_id: rental.id, kind: 'message', actor_id: null,
        payload: { text: "It's yours now — you've got an hour to flag anything that doesn't match the listing. After that, this settles automatically, no return needed.", system: true },
      });
    }
  ...
  if (kind === 'start_return') {
    ...
    if (rental.transaction_type === 'buy') return NextResponse.json({ error: 'This was a purchase — there\'s nothing to return' }, { status: 400 });
  ...
  if (kind === 'report_no_show') {
    ...
    const admin = supabaseAdmin();
    const { data: p } = await admin.from('profiles').select('no_show_count').eq('id', noShowUserId).single();
    await admin.from('profiles').update({ no_show_count: (p?.no_show_count || 0) + 1 }).eq('id', noShowUserId);
  ...
  if (kind === 'dispute_no_show') {
    ...
    const adminForDispute = supabaseAdmin();
    const { data: p } = await adminForDispute.from('profiles').select('no_show_count').eq('id', accusedUserId).single();
    await adminForDispute.from('profiles').update({ no_show_count: Math.max(0, (p?.no_show_count || 1) - 1) }).eq('id', accusedUserId);
...
async function createPaymentHolds(supabase, rental) {
  ...
  const isBuy = rental.transaction_type === 'buy';
  const basePrice = isBuy ? listing.buy_price_cents : listing.price_cents;

  const eligible = await referralEligible(supabase, renterProfile);
  const rentalChargeAmount = eligible ? Math.round(basePrice * 0.95) : basePrice;
  const applicationFee = eligible ? Math.round(basePrice * 0.03) : Math.round(basePrice * 0.15);
  ...
  // a purchase has no deposit — there's nothing to give back since the item
  // isn't coming back
  const depositIntent = isBuy ? null : await stripe.paymentIntents.create({
  ...
  try {
    ...
    if (pm?.card?.fingerprint) await supabaseAdmin().from('profiles').update({ card_fingerprint: pm.card.fingerprint }).eq('id', renterProfile.id);
  } catch {}
  ...
  return { rentalIntentId: rentalIntent.id, depositIntentId: depositIntent?.id || null };
}
...
async function creditReferrerIfEligible(supabase, rental) {
  ...
  const listing = rental.listings;
  const basePrice = rental.transaction_type === 'buy' ? listing.buy_price_cents : listing.price_cents;
  const credit = Math.round(basePrice * 0.05);
  const capped = Math.min((referrer.referral_credit_cents || 0) + credit, 5000); // $50 cap
  await supabaseAdmin().from('profiles').update({ referral_credit_cents: capped, referral_credit_earned_at: new Date().toISOString() }).eq('id', referrer.id);
