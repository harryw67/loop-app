...
export async function POST(req) {
  ...
  if (mode === 'buy') {
    if (!listing.purchasable) return NextResponse.json({ error: 'This item is not available to buy' }, { status: 400 });
    if (listing.sold) return NextResponse.json({ error: 'This item has already been sold' }, { status: 400 });
  }
  if (mode === 'book' && !listing.rentable) {
    return NextResponse.json({ error: 'This item is not available to rent' }, { status: 400 });
  }
  ...
  const transaction_type = mode === 'buy' ? 'buy' : 'rent';
  ...
  const { data: rental, error } = await supabase
    ...
    .insert({
      ...
      qr_token,
      transaction_type,
      start_date: mode === 'book' ? start_date : null,
      end_date: mode === 'book' ? end_date : null,
      expires_at,
    })
    .select()
    .single();
