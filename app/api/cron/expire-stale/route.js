...
export async function GET(req) {
  ...
  // 3b. purchases auto-settle once the 1-hour "flag an issue" window closes,
  // since there's no return step for an outright sale — same window used
  // for the renter's pre-existing-damage photos on a normal rental
  const ONE_HOUR_MS = 60 * 60 * 1000;
  const { data: pendingBuys } = await supabase.from('rentals').select('*').eq('stage', 'out').eq('transaction_type', 'buy');
  let buysSettled = 0;
  for (const rental of pendingBuys || []) {
    if (rental.handoff_confirmed_at && Date.now() - new Date(rental.handoff_confirmed_at).getTime() >= ONE_HOUR_MS) {
      await supabase.from('rentals').update({ stage: 'settled' }).eq('id', rental.id);
      await supabase.from('rental_events').insert({
        rental_id: rental.id, kind: 'message', actor_id: null,
        payload: { text: "The flag-an-issue window closed with no issues raised — this purchase is now settled.", system: true },
      });
      buysSettled++;
    }
  }
  results.buysSettled = buysSettled;
