...
function BrowsePageInner() {
  ...
  }, [sizes, categories, genders, colors, maxPrice, maxDistance, search, myLocation, availFrom, availTo, occasion]);
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div className="serif" style={{ fontSize: 36, fontWeight: 600, lineHeight: 1.05, color: 'var(--ink)' }}>Wear more.</div>
          <div className="serif" style={{ fontSize: 36, fontWeight: 600, lineHeight: 1.05, color: 'var(--oxblood)' }}>Own less.</div>
          <div style={{ fontSize: 14, color: 'var(--ink-soft)', margin: '10px 0 16px' }}>
            The best closets on campus, now yours.
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a href="#browse-results" style={{ background: 'var(--oxblood)', color: 'var(--oxblood-bg)', fontSize: 13, fontWeight: 500, padding: '9px 16px', borderRadius: 999, textDecoration: 'none' }}>
              Browse the drop →
            </a>
            <button onClick={() => router.push('/listings/new')} style={{ background: 'var(--ink)', color: 'var(--white)', fontSize: 13, fontWeight: 500, padding: '9px 16px', borderRadius: 999, border: 'none' }}>
              List an item
            </button>
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 220, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div style={{ gridRow: 'span 2', background: '#F0997B', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 44 }}>🧥</div>
          <div style={{ background: 'var(--mustard-bg)', borderRadius: 12, aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>👟</div>
          <div style={{ background: '#7F77DD', borderRadius: 12, aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>👗</div>
        </div>
      </div>

      <div style={{ background: 'var(--mustard-bg)', borderRadius: 14, padding: 16, marginBottom: 28, display: 'flex', gap: 10 }}>
        {[['1', 'Browse & request'], ['2', 'Meet up & confirm'], ['3', 'Wear & return']].map(([n, label]) => (
          <div key={n} style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ fontSize: 17, fontWeight: 600, color: 'var(--oxblood)' }}>{n}</div>
            <div style={{ fontSize: 11.5, color: 'var(--mustard-ink)' }}>{label}</div>
          </div>
        ))}
      </div>
      ...
      {featured.length > 0 && (
        <div style={{ marginBottom: 34 }}>
          <div className="serif" style={{ fontSize: 20, fontWeight: 500, marginBottom: 14 }}>
            Trending near you
          </div>
          <div className="featured-grid" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
            {featured[0] && (
              <div onClick={() => router.push(`/listings/${featured[0].id}`)} style={{ cursor: 'pointer', position: 'relative' }}>
                <PhotoTile url={featured[0].photo_url} style={{ aspectRatio: '4/3', borderRadius: 10 }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '22px 16px 14px', background: 'linear-gradient(to top, rgba(34,30,25,0.72), transparent)', borderRadius: '0 0 10px 10px' }}>
                  <div style={{ color: 'var(--white)', fontSize: 16, fontWeight: 500 }}>{featured[0].name}</div>
                  <div style={{ color: '#F5C4B3', fontSize: 13, marginTop: 2 }}>
                    {featured[0].purchasable ? `$${(featured[0].buy_price_cents / 100).toFixed(0)} to buy` : `$${(featured[0].price_cents / 100).toFixed(0)}/day`}
                  </div>
                </div>
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {featured.slice(1, 3).map(l => (
                <div key={l.id} onClick={() => router.push(`/listings/${l.id}`)} style={{ cursor: 'pointer', position: 'relative', flex: 1 }}>
                  <PhotoTile url={l.photo_url} style={{ aspectRatio: '16/9', borderRadius: 10, height: '100%' }} />
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '14px 12px 8px', background: 'linear-gradient(to top, rgba(34,30,25,0.72), transparent)', borderRadius: '0 0 10px 10px' }}>
                    <div style={{ color: 'var(--white)', fontSize: 13, fontWeight: 500 }}>{l.name}</div>
                    <div style={{ color: '#F5C4B3', fontSize: 11.5 }}>${(l.price_cents / 100).toFixed(0)}/day</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: 28 }}>
        {[
          { label: 'Game day', bg: '#3A1740' },
          { label: 'Going out', bg: '#5C1F1F' },
          { label: 'Special events', bg: '#1F3A3A' },
          { label: 'Everyday', bg: '#3A2E1A' },
          { label: 'Formal', bg: '#1A2A3A' },
        ].map(t => {
          const active = occasion === t.label;
          return (
            <button
              key={t.label}
              onClick={() => setOccasion(active ? '' : t.label)}
              style={{
                position: 'relative', background: t.bg, borderRadius: 10, height: 76, display: 'flex', alignItems: 'flex-end',
                padding: 12, border: active ? '2px solid var(--oxblood)' : '2px solid transparent', textAlign: 'left',
              }}
            >
              <span style={{ color: '#fff', fontSize: 13.5, fontWeight: 600 }}>{t.label}</span>
            </button>
          );
        })}
      </div>

      <div id="browse-results" style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
        <div>
          <h1 className="serif" style={{ fontSize: 24, fontWeight: 500 }}>{occasion ? occasion : 'Browse'}</h1>
          <p style={{ color: 'var(--ink-soft)', fontSize: 14, marginTop: 4 }}>
            {loading ? 'Loading…' : `${listings.length} piece${listings.length === 1 ? '' : 's'} available`}
          </p>
        </div>
        <input
          value={search}
      ...
      <div style={{ marginBottom: 22 }}>
        <div style={{ display: 'flex', gap: 8, position: 'relative', flexWrap: 'wrap', justifyContent: 'space-between' }}>
          ...
          <Dropdown label={`Sort: ${SORTS.find(s => s.value === sortBy)?.label}`} openKey="sort" open={open} setOpen={setOpen} active={sortBy !== 'newest'} align="right">
            {SORTS.map(s => (
          ...
          >
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: 8, left: 8, display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-start' }}>
                {isNew(l.created_at) && (
                  <span style={{ fontSize: 10.5, fontWeight: 600, padding: '3px 8px', borderRadius: 999, background: 'var(--sage-bg)', color: 'var(--sage-ink)' }}>
                    NEW
                  </span>
                )}
                {l.purchasable && (
                  <span style={{ fontSize: 10.5, fontWeight: 600, padding: '3px 8px', borderRadius: 999, background: 'var(--mustard-bg)', color: 'var(--mustard-ink)' }}>
                    BUY
                  </span>
                )}
                {(l.favorites?.[0]?.count || 0) >= 3 && (
                  <span style={{ fontSize: 10.5, fontWeight: 600, padding: '3px 8px', borderRadius: 999, background: 'var(--oxblood-bg)', color: 'var(--oxblood-ink)' }}>
                    POPULAR
                  </span>
                )}
              </div>
      ...
      <div style={{ background: 'var(--oxblood)', borderRadius: 14, padding: '18px 22px', marginTop: 40, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div className="serif" style={{ fontSize: 17, fontWeight: 500, color: 'var(--oxblood-bg)' }}>Your closet, expanded.</div>
          <div style={{ fontSize: 12, color: '#F5C4B3' }}>Real people. Real pieces. A more circular campus.</div>
        </div>
        <button onClick={() => router.push('/listings/new')} style={{ background: 'var(--mustard-bg)', color: 'var(--mustard-ink)', fontSize: 12.5, fontWeight: 500, padding: '8px 16px', borderRadius: 999, border: 'none', whiteSpace: 'nowrap' }}>
...
function Dropdown({ label, openKey, open, setOpen, active, children, align = 'left' }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {isOpen && (
        <div style={{
          position: 'absolute', top: 'calc(100% + 8px)', [align]: 0, zIndex: 20, background: 'var(--white)',
          border: '1px solid var(--line)', borderRadius: 6, padding: '14px 16px', minWidth: 190,
          maxWidth: 'calc(100vw - 32px)', boxShadow: '0 6px 18px rgba(34,30,25,0.08)',
        }}>
          {children}
        </div>
      )}
