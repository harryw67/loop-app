...
export default function AdminPage() {
  ...
  const [itemDisputes, setItemDisputes] = useState([]);
  ...
  const load = async () => {
    ...
    fetch('/api/admin/disputes').then(r => r.json()).then(d => setItemDisputes(d.disputes || []));
  };

  const resolveItemDispute = async (rental_id, action) => {
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      if (!res.ok) { setErrorMsg(d.error || `Search failed (${res.status})`); setUsers([]); return; }
      setUsers(d.users || []);
    } catch {
  ...
  const searchListings = async () => {
    setErrorMsg('');
    try {
      const res = await fetch(`/api/admin/listings?q=${encodeURIComponent(listingQuery)}`);
      const d = await res.json();
      if (!res.ok) { setErrorMsg(d.error || `Search failed (${res.status})`); setListings([]); return; }
  ...
  const takeDownListing = async (id) => {
    if (!confirm('Take down this listing? It will no longer show up in Browse.')) return;
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 22, flexWrap: 'wrap' }}>
        {['reports', 'disputes', 'item-disputes', 'users', 'listings'].map(t => (
          <button key={t} onClick={() => setTab(t)} className={t === tab ? 'btn btn-primary' : 'btn btn-ghost'} style={{ fontSize: 13 }}>
            {t === 'disputes' ? 'No-show disputes' : t === 'item-disputes' ? 'Item disputes' : t.charAt(0).toUpperCase() + t.slice(1)}
            {t === 'reports' && reports.length ? ` (${reports.length})` : ''}
            {t === 'disputes' && disputes.length ? ` (${disputes.length})` : ''}
            {t === 'item-disputes' && itemDisputes.length ? ` (${itemDisputes.length})` : ''}
          </button>
        ))}
      </div>
      ...
      {tab === 'item-disputes' && (
        <div>
          {itemDisputes.length === 0 && <p style={{ color: 'var(--ink-faint)', fontSize: 13.5 }}>No item disputes awaiting review.</p>}
          {itemDisputes.map(d => (
            <div key={d.id} style={{ border: '1px solid var(--line)', borderRadius: 4, padding: 14, marginBottom: 10 }}>
              <div style={{ fontSize: 13.5 }}>{d.listings?.name} — {d.owner?.full_name} / {d.renter?.full_name}</div>
              <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 4 }}>Reason: {d.dispute_reason || 'No reason given'}</div>
              {d.photos?.length > 0 && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                  {d.photos.map((p, i) => (
                    <img key={i} src={p.payload?.url} alt="" style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 4 }} />
                  ))}
                </div>
              )}
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <button className="btn btn-ghost" style={{ fontSize: 12.5 }} onClick={() => resolveItemDispute(d.id, 'refund_renter')}>Refund renter (release deposit)</button>
                <button className="btn btn-ghost" style={{ fontSize: 12.5 }} onClick={() => resolveItemDispute(d.id, 'capture_deposit')}>Capture deposit (owner compensation)</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'listings' && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            <input value={listingQuery} onChange={e => setListingQuery(e.target.value)} placeholder="Search listing name" style={{ flex: 1, padding: 8, border: '1px solid var(--line)', borderRadius: 4 }} onKeyDown={e => e.key === 'Enter' && searchListings()} />
            <button className="btn btn-ghost" onClick={searchListings}>Search</button>
          </div>
          {listings.map(l => (
            <div key={l.id} style={{ border: '1px solid var(--line)', borderRadius: 4, padding: 14, marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 500 }}>{l.name}</div>
                <div style={{ fontSize: 11.5, color: 'var(--ink-faint)' }}>
                  ${(l.price_cents / 100).toFixed(0)}/day · listed by {l.profiles?.full_name}{l.profiles?.username && ` (@${l.profiles.username})`}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <a href={`/listings/${l.id}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost" style={{ fontSize: 12.5, textDecoration: 'none' }}>View</a>
                <button className="btn btn-ghost" style={{ fontSize: 12.5, color: 'var(--oxblood)' }} onClick={() => takeDownListing(l.id)}>Take down</button>
              </div>
            </div>
          ))}
        </div>
