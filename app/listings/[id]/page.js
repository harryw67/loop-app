...
export default function ListingDetail({ params }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      setMe(user);
      if (user) {
        const { data: existingFav } = await supabase.from('favorites').select('id').eq('user_id', user.id).eq('listing_id', params.id).maybeSingle();
        setFavorited(!!existingFav);
      }
    });
    ...
    supabase.from('listings').select('*, profiles(full_name)').eq('id', params.id).eq('active', true).maybeSingle()
      .then(async ({ data, error }) => {
        ...
        const { data: ownerProfile } = await supabase.from('profiles').select('no_show_count').eq('id', data.owner_id).maybeSingle();
  ...
  const shareListing = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: listing.name, text: `Check out this ${listing.name} on Loop`, url }); } catch {}
    } else {
    ...
    <div className="listing-detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
      <div>
        <div style={{ aspectRatio: '3/4', borderRadius: 4, position: 'relative' }}>
          ...
          <button onClick={toggleFavorite} style={{ position: 'absolute', top: 12, right: 12, width: 36, height: 36, borderRadius: '50%', background: 'rgba(253,251,245,0.92)', border: 'none', fontSize: 17, color: favorited ? 'var(--mustard-ink)' : 'var(--ink-faint)', transition: 'color 0.15s ease' }}>
            {favorited ? '★' : '☆'}
          </button>
          <button onClick={shareListing} style={{ position: 'absolute', top: 12, right: 56, width: 36, height: 36, borderRadius: '50%', background: 'rgba(253,251,245,0.92)', border: 'none', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Share">
            ↗
          </button>
          {shareCopied && (
            <span style={{ position: 'absolute', top: 52, right: 12, fontSize: 11, background: 'rgba(34,30,25,0.85)', color: 'white', padding: '4px 8px', borderRadius: 4 }}>
              Link copied
            </span>
          )}
        </div>
      ...
      <div>
        ...
        <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 6 }}>
          Listed by {listing.profiles?.full_name || 'a student'}{listing.gender && ` · ${listing.gender}`} · size {listing.size}{listing.color && ` · ${listing.color}`}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
          {listing.category && (
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => router.push(`/?categories=${encodeURIComponent(listing.category)}`)}>
              More {listing.category}
            </button>
          )}
          {listing.gender && (
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => router.push(`/?genders=${encodeURIComponent(listing.gender)}`)}>
              More {listing.gender}
            </button>
          )}
          {listing.color && (
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => router.push(`/?colors=${encodeURIComponent(listing.color)}`)}>
              More in {listing.color}
            </button>
          )}
        </div>
        <div style={{ marginTop: 22, paddingTop: 18, borderTop: '1px solid var(--line)' }}>
          {listing.sold ? (
            <span style={{ display: 'inline-block', fontSize: 11.5, fontWeight: 500, padding: '4px 10px', borderRadius: 999, marginBottom: 10, background: 'var(--oxblood-bg)', color: 'var(--oxblood-ink)' }}>
              Sold
            </span>
          ) : availabilityBadge && (
            <span style={{
          ...
          {listing.rentable && (
            <div style={{ marginBottom: listing.purchasable ? 10 : 0 }}>
              <span style={{ fontSize: 26, fontFamily: 'Fraunces, serif', fontWeight: 500 }}>${(listing.price_cents / 100).toFixed(0)}</span>
              <span style={{ fontSize: 13, color: 'var(--ink-faint)' }}> / day to rent</span>
          ...
          {listing.purchasable && (
            <div>
              <span style={{ fontSize: 22, fontFamily: 'Fraunces, serif', fontWeight: 500 }}>${(listing.buy_price_cents / 100).toFixed(0)}</span>
              <span style={{ fontSize: 13, color: 'var(--ink-faint)' }}> to buy outright</span>
            </div>
          )}
        </div>
        ...
        {isOwner ? (
          ...
          <p style={{ marginTop: 20, fontSize: 13, color: 'var(--ink-faint)', textAlign: 'center' }}>This item has been sold and is no longer available.</p>
        ) : (
          <>
            {listing.rentable && (
              <>
                ...
                {startDate && endDate && endDate >= startDate && (
                  <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', marginTop: 8 }}>
                    {Math.round((new Date(endDate) - new Date(startDate)) / 86400000) + 1} day(s) — total ${(((Math.round((new Date(endDate) - new Date(startDate)) / 86400000) + 1) * listing.price_cents) / 100).toFixed(0)}
                  </p>
                )}
                {booked.length > 0 && (
            ...
            {listing.purchasable && (
              <button className={listing.rentable ? 'btn btn-ghost btn-block' : 'btn btn-primary btn-block'} style={{ marginTop: 10 }} onClick={() => start('buy')}>
                Buy now — ${(listing.buy_price_cents / 100).toFixed(0)}
              </button>
            )}
