...
export default function EditListingPage({ params }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      setForm({
        name: data.name, category: data.category, size: data.size, color: data.color || 'Black', gender: data.gender || "Women's", occasion: data.occasion || 'Everyday',
        ...
        min_days: (data.min_days || 1).toString(), max_days: (data.max_days || 14).toString(),
        rentable: data.rentable !== false, purchasable: !!data.purchasable, buy_price: data.buy_price_cents ? (data.buy_price_cents / 100).toString() : '',
      });
