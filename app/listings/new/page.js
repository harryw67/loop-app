...
const OCCASIONS = ['Everyday', 'Going out', 'Game day', 'Special events', 'Formal'];
const COLORS = ['Black', 'White', 'Red', 'Pink', 'Orange', 'Yellow', 'Green', 'Blue', 'Purple', 'Brown', 'Beige / Tan', 'Gold / Silver', 'Multicolor'];
const SIZES = ['XS', 'S', 'M', 'L', 'XL'];

export default function NewListingPage() {
  const router = useRouter();
  const [form, setForm] = useState({ name: '', category: 'Dress', size: 'S', color: 'Black', gender: "Women's", occasion: 'Everyday', price: '', deposit: '', description: '', care_instructions: '', min_days: '1', max_days: '6', rentable: true, purchasable: false, buy_price: '' });
  ...
  const submit = async () => {
    setError('');
    const missing = [];
    if (!form.name) missing.push('a name');
    if (!form.description) missing.push('a description');
    if (missing.length) {
      setError(`Fill in ${missing.join(' and ')} before publishing.`);
      return;
    }
    if (!form.rentable && !form.purchasable) {
      setError('Make this available to rent, to buy, or both.');
      return;
    }
    if (form.rentable && (!form.price || !form.deposit)) {
      setError('Fill in a price per day and deposit for renting.');
      return;
    }
    if (form.rentable && parseFloat(form.deposit) < parseFloat(form.price) * 2) {
      setError('Deposit should be at least 2x the daily price — helps cover real damage costs.');
      return;
    }
    if (form.purchasable && !form.buy_price) {
      setError('Fill in a buy-it-now price.');
      return;
    }
    const supabase = supabaseBrowser();
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      <div className="field" style={{ marginBottom: 18 }}>
        ...
        <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="Camel wrap midi" required />
      </div>
      ...
      <div style={{ marginBottom: 18 }}>
        <label style={{ display: 'block', fontSize: 12.5, color: 'var(--ink-soft)', marginBottom: 8 }}>How is this available?</label>
        <div style={{ display: 'flex', gap: 18 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13.5, cursor: 'pointer' }}>
            <input type="checkbox" checked={form.rentable} onChange={e => set('rentable', e.target.checked)} /> To rent
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13.5, cursor: 'pointer' }}>
            <input type="checkbox" checked={form.purchasable} onChange={e => set('purchasable', e.target.checked)} /> To buy outright
          </label>
        </div>
      </div>
        ...
        <>
          <div style={{ display: 'flex', gap: 14, marginBottom: 6, flexWrap: 'wrap' }}>
            <div className="field" style={{ flex: 1 }}>
              ...
              <input value={form.price} onChange={e => set('price', e.target.value)} placeholder="22" required={form.rentable} />
            </div>
            <div className="field" style={{ flex: 1 }}>
              <label>Deposit</label>
              <input value={form.deposit} onChange={e => set('deposit', e.target.value)} placeholder="40" required={form.rentable} />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 14, marginBottom: 6, flexWrap: 'wrap' }}>
            ...
            <div className="field" style={{ flex: 1 }}>
              ...
              <input type="number" min="1" max="6" value={form.max_days} onChange={e => set('max_days', e.target.value)} />
            </div>
          </div>
          <p style={{ fontSize: 11.5, color: 'var(--ink-faint)', marginBottom: 18 }}>
            Capped at 6 days — longer than that risks the deposit hold on the renter's card expiring before the item is returned.
          </p>
        </>
      )}
      ...
      {form.purchasable && (
        <div className="field" style={{ marginBottom: 18 }}>
          ...
          <input value={form.buy_price} onChange={e => set('buy_price', e.target.value)} placeholder="60" />
        </div>
      )}
