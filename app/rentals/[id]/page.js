    ...
    <div style={{ maxWidth: 520, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {EXCEPTION_STAGES.includes(rental.stage) ? (
            <span style={{ fontSize: 11, padding: '4px 9px', borderRadius: 999, border: '1px solid var(--oxblood)', background: 'var(--oxblood-bg)', color: 'var(--oxblood-ink)' }}>
              {STAGE_LABEL[rental.stage]}
            </span>
          ) : STAGES.map((s, i) => {
      ...
      <div style={{ border: '1px solid var(--line)', borderRadius: 4, background: 'var(--white)', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--line)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-faint)' }}>
            {rental.transaction_type === 'buy'
              ? `$${(rental.listings.buy_price_cents / 100).toFixed(0)} — buy`
              : <>${(rental.listings.price_cents / 100).toFixed(0)}/day · ${(rental.listings.deposit_cents / 100).toFixed(0)} deposit</>}
            {rental.start_date && ` · ${formatDateShort(rental.start_date)} to ${formatDateShort(rental.end_date)}`}
          </div>
        </div>
        ...
        <div ref={bodyRef} style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12, maxHeight: '56vh', overflowY: 'auto' }}>
          ...
          {rental.stage === 'inquiry' && !isOwner && rental.transaction_type === 'buy' && (
            <SysBlock label="Just messaging for now — buy when you're ready">
              <button className="btn btn-primary" onClick={() => post({ kind: 'request_to_rent' })}>
                Request to buy — ${(rental.listings.buy_price_cents / 100).toFixed(0)}
              </button>
            </SysBlock>
          )}
          {rental.stage === 'inquiry' && !isOwner && rental.transaction_type !== 'buy' && (
            <SysBlock label="Just messaging for now — pick dates when you're ready to book">
              ...
              {reqStartDate && reqEndDate && reqEndDate >= reqStartDate && (
                <p style={{ fontSize: 12, color: 'var(--ink-soft)', marginBottom: 10 }}>
                  {Math.round((new Date(reqEndDate) - new Date(reqStartDate)) / 86400000) + 1} day(s) — total ${(((Math.round((new Date(reqEndDate) - new Date(reqStartDate)) / 86400000) + 1) * rental.listings.price_cents) / 100).toFixed(0)}
                </p>
              )}
              <button
                ...
                onClick={() => {
                  ...
                  if (reqEndDate < reqStartDate) { setBanner('End date is before the start date.'); return; }
            ...
            <SysBlock label="This renter wants to book — approve or decline">
              {!ownerPayoutReady && (
                <p style={{ fontSize: 11.5, color: 'var(--oxblood)', marginBottom: 10 }}>
                  Heads up: you haven't connected a payout account yet, so if you approve this, you'll be paid out manually for now instead of automatically — <a href="/profile" style={{ color: 'var(--oxblood)' }}>connect one here</a> when you're ready.
                </p>
              )}
              {renterStats && (
                <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', marginBottom: 10 }}>
                  {renterStats.avgRating ? `${renterStats.avgRating}★ (${renterStats.reviewCount})` : 'No reviews yet'}
                  {renterStats.noShows > 0 && <span style={{ color: 'var(--oxblood)' }}> · {renterStats.noShows} no-show{renterStats.noShows === 1 ? '' : 's'}</span>}
                  {renterStats.cancellations > 0 && <span> · {renterStats.cancellations} cancellation{renterStats.cancellations === 1 ? '' : 's'}</span>}
                </div>
              )}
          ...
          {rental.stage === 'pending' && !isOwner && (
            <SysBlock label="Waiting on the owner to approve your request">
              {!hasCard && (
                <p style={{ fontSize: 11.5, color: 'var(--oxblood)', marginBottom: 0 }}>
                  Add a card now so payment can be held the moment they approve — <a href="/payment-method" style={{ color: 'var(--oxblood)', textDecoration: 'underline' }}>add one here</a>.
                </p>
              )}
              <div style={{ marginTop: 20 }}>
                <button className="btn btn-ghost" style={{ fontSize: 12 }} onClick={cancelBooking}>Cancel request</button>
              </div>
            </SysBlock>
          )}
          ...
          {rental.stage === 'booked' && (
            <SysBlock label="Where to meet up?">
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 10 }}>
                {['campus library', 'student union', 'a coffee shop', 'dorm lobby'].map(place => (
                  <button key={place} className="btn btn-ghost" style={{ fontSize: 11.5, padding: '5px 10px' }} onClick={() => suggestPlace(place)}>{place}</button>
                ))}
              </div>
              {meetupStatus !== 'done' && (
                <button className="btn btn-ghost" style={{ fontSize: 12 }} onClick={suggestMeetupSpot} disabled={meetupStatus === 'requesting'}>
                  {meetupStatus === 'requesting' ? 'Finding a midpoint…' : '📍 Suggest a spot halfway between us'}
                </button>
              )}
              {meetupStatus === 'no-owner-location' && <p style={{ fontSize: 11, color: 'var(--ink-faint)', marginTop: 6 }}>Owner didn't share a location for this listing, so a midpoint can't be calculated — try one of the suggestions above instead.</p>}
              {meetupStatus === 'denied' && <p style={{ fontSize: 11, color: 'var(--ink-faint)', marginTop: 6 }}>Couldn't get your location — try again or use a suggestion above.</p>}
              {meetupStatus === 'done' && meetupSpot && (
                <div>
                  <a href={`https://www.google.com/maps?q=${meetupSpot.lat},${meetupSpot.lng}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12.5, color: 'var(--oxblood)' }}>Open suggested midpoint in Maps ↗</a>
                  <div style={{ marginTop: 6 }}>
                    <button className="btn btn-ghost" style={{ fontSize: 11.5 }} onClick={shareMeetupSpot}>Share this in chat</button>
                  </div>
                </div>
              )}
            </SysBlock>
          ...
          {rental.stage === 'booked' && !isOwner && (
            <SysBlock label="Meet up, then enter the code the owner shows you">
              {!hasCard && (
                <p style={{ fontSize: 11.5, color: 'var(--oxblood)', marginBottom: 16 }}>
                  You'll need a card on file first — <a href="/payment-method" style={{ color: 'var(--oxblood)', textDecoration: 'underline' }}>add one here</a>.
                </p>
              )}
              <input
          ...
          {rental.stage === 'booked' && rental.start_date && new Date().toISOString().slice(0, 10) >= rental.start_date && (
            <button
              style={{ fontSize: 12, color: 'var(--oxblood)', alignSelf: 'center' }}
              onClick={() => {
                if (confirm(`Report that ${other?.full_name} never showed up for the handoff? This cancels any payment hold and adds to their no-show record.`)) {
                  post({ kind: 'report_no_show' });
                }
              }}
            >
              {other?.full_name} never showed up
            </button>
          )}
          {rental.stage === 'booked' && (
            <button style={{ fontSize: 11.5, color: 'var(--ink-faint)', alignSelf: 'center' }} onClick={cancelBooking}>Cancel this booking</button>
          )}

          {rental.stage === 'out' && !isOwner && windowOpen && (
            <SysBlock label={rental.transaction_type === 'buy' ? `Notice anything not as described? Flag it — closes in ${minutesLeft} min` : `Notice anything already wrong? Document it — closes in ${minutesLeft} min`}>
              <p style={{ fontSize: 11.5, color: 'var(--ink-faint)', marginBottom: 10 }}>
                {rental.transaction_type === 'buy'
                  ? "The owner's listing photos are the baseline — this is your chance to flag anything that doesn't match before this settles for good."
                  : "The owner's listing photos are the baseline — this is just your chance to flag anything that doesn't match before you wear it."}
              </p>
          ...
          {rental.stage === 'out' && rental.transaction_type === 'buy' && (
            <p style={{ fontSize: 11.5, color: 'var(--ink-faint)', textAlign: 'center' }}>
              {windowOpen ? "This settles automatically once the window above closes — no return needed." : "This has settled — sale complete."}
            </p>
          )}
          {rental.stage === 'out' && rental.transaction_type !== 'buy' && !isOwner && (
            <button className="btn btn-primary" onClick={() => post({ kind: 'start_return' })}>
              I'm returning this now
            </button>
          )}
          {rental.stage === 'out' && rental.transaction_type !== 'buy' && isOwner && (
            <button className="btn btn-ghost" onClick={() => post({ kind: 'message', text: "Ready whenever you are to return it — just hit the button on your end when you're headed my way." })}>
              Nudge about return
            </button>
          )}
          ...
          {rental.stage === 'return' && (
            <SysBlock label="Condition photos — at return">
              ...
              />
              <button style={{ fontSize: 12, color: 'var(--oxblood)', marginTop: 10 }} onClick={() => { const reason = prompt("What's wrong?"); if (reason) post({ kind: 'dispute', reason }); }}>
                Raise an issue instead
              </button>
            </SysBlock>
          )}

          {rental.stage === 'no_show' && (
            <SysBlock label="This rental was marked as a no-show — any payment hold was cancelled">
              {(() => {
                const reportEvent = events.find(e => e.kind === 'no_show');
                const accusedRole = reportEvent?.payload?.reported;
                const iWasAccused = accusedRole === role;
                const stillOpen = rental.no_show_status === 'confirmed' && rental.no_show_dispute_deadline && new Date() < new Date(rental.no_show_dispute_deadline);
                if (rental.no_show_status === 'under_review') return <p style={{ fontSize: 11.5, color: 'var(--mustard-ink)' }}>This report is disputed and under admin review.</p>;
                if (!iWasAccused || !stillOpen) return null;
                return (
                  <div>
                    <textarea value={noShowDisputeReason} onChange={e => setNoShowDisputeReason(e.target.value)} placeholder="What actually happened?" style={{ width: '100%', padding: 8, border: '1px solid var(--line)', borderRadius: 4, fontSize: 13, minHeight: 50, marginBottom: 8 }} />
                    <button className="btn btn-ghost" style={{ fontSize: 12.5 }} onClick={disputeNoShow}>Dispute this report</button>
                  </div>
                );
              })()}
            </SysBlock>
          )}
          {rental.stage === 'expired' && (
            <SysBlock label="This booking's dates passed with no handoff — automatically cancelled" />
          )}
          {rental.stage === 'cancelled' && (
            <SysBlock label={rental.cancelled_by ? `Cancelled by the ${rental.cancelled_by}` : 'This booking was cancelled'} />
          )}

          {(rental.stage === 'settled' || rental.stage === 'disputed' || rental.stage === 'no_show') && (
            <SysBlock label={myReview ? 'Your review' : `Rate renting with ${other?.full_name}`} confirmed={rental.stage === 'settled'}>
              {myReview ? (
...
function ConfirmRow({ phase, role, confirmed, onConfirm }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      {!iConfirmed && <button className="btn btn-ghost" style={{ fontSize: 12.5 }} onClick={onConfirm}>Looks good</button>}
    </div>
  );
}
