...
export default function RentalsPage() {
  ...
  useEffect(() => {
    const load = () => fetch('/api/rentals').then(r => r.json()).then(d => setRentals(d.rentals || [])).finally(() => setLoading(false));
    load();
    // keeps this list from showing a stale status if something changed
    // elsewhere (e.g. you cancelled something in a thread and navigated back)
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--line)', border: '1px solid var(--line)' }}>
        {rentals.map(r => (
          <Link key={r.id} href={`/rentals/${r.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div className="list-row" style={{ background: 'var(--white)', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer' }}>
              ...
              <span style={{ color: 'var(--ink-faint)', fontSize: 16, marginLeft: 2 }}>›</span>
            </div>
          </Link>
        ))}
      </div>
