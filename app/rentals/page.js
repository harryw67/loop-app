        ...
        {rentals.map(r => (
          <Link key={r.id} href={`/rentals/${r.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ background: 'var(--white)', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 44, height: 56, flexShrink: 0, background: r.listings?.photo_url ? `url(${r.listings.photo_url}) center/cover` : 'var(--cream-2)' }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14.5, fontWeight: 500 }}>{r.listings?.name}</div>
                <div style={{ fontSize: 12.5, color: 'var(--ink-faint)', marginTop: 2 }}>
                  with {r.owner?.full_name || r.renter?.full_name} · {STAGE_LABEL[r.stage]}
                </div>
              </div>
              <span style={{ fontSize: 11.5, padding: '4px 10px', borderRadius: 999, fontWeight: 500, background: 'var(--mustard-bg)', color: 'var(--mustard-ink)' }}>
                {STAGE_LABEL[r.stage]}
              </span>
            </div>
          </Link>
